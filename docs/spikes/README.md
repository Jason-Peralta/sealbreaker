...
| S3 portal and dimension | Does the vanilla Portal interface work for a custom gate into a JSON dimension with its own world clock? | not started |
| S4 item presentation | Do item model definitions plus a custom data component render a reforge prefix, tooltip line and glint? | not started |
| S5 structure | Does a jigsaw structure with a processor list and a locked-door block spawn at a configured rarity? | not started |
