...
- [~] 3. CI workflow written (`.github/workflows/build.yml`: build + `gameTests`, on the local branch `ci/workflow`) and `.github/CODEOWNERS` merged; the workflow cannot be pushed by this session's token (no `workflow` scope: run `gh auth refresh -h github.com -s workflow`, then `git push origin ci/workflow` and merge). Merges are squash-only with branch deletion (repo setting). Branch protection and rulesets are refused on a free-plan private repository (GitHub Pro or a public repo unlocks them): until one of those, the review rule is a convention backed by CODEOWNERS review requests (#2).
