...
- [~] 3. CI workflow written (`.github/workflows/build.yml`: build + `gameTests`, on the local branch `ci/workflow`) and `.github/CODEOWNERS` merged; the workflow cannot be pushed by this session's token (no `workflow` scope: run `gh auth refresh -h github.com -s workflow`, then `git push origin ci/workflow` and merge). Merges are squash-only with branch deletion (repo setting). The repository went public on 6 Sep 2026 (decision 0021) and main is protected: pull requests with code-owner review for the CODEOWNERS paths, no force pushes; the build workflow becomes a required check after its first run (#2).
...
- [~] 5. JBR hot-swap: the run configurations carry `-XX:+IgnoreUnrecognizedVMOptions -XX:+AllowEnhancedClassRedefinition`; IntelliJ 2026.1.1 bundles JBR 25.0.2, select it as the run JRE to activate (not yet exercised by a person). The resource half of the loop is verified without a person: `:sb_combat:runClientAnim -Psb.animdebug=reload` edits the sword animation where the run reads it, presses F3+T and reads the new length back (5 Sep 2026); the code hot-swap half needs the IDE and a person (#4).
- [~] 6. packwiz pack initialised 6 Sep 2026 (NeoForge 26.2.0.76; GeckoLib 5.5.5, Curios 16.0.0, JEI 30.31 from Modrinth; our five jars from the `v0.0.1` pre-release; `packwiz serve` and a server-side `packwiz-installer` dry run resolved all eight files; `pack/server/start.sh` installs a throwaway server). Left: the Prism Launcher import and the throwaway server join with a hand-accepted EULA (#3).
